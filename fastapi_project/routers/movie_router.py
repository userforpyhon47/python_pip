...
from models.base_model import MovieItem

# Create router app
movie_router = APIRouter()

# Endpoint for getting all movies
@movie_router.get("/movies", tags=["Movies"], response_model=List[MovieItem], dependencies=[Depends(JWTBearer())])
def list_movies():
...
@movie_router.get("/movies/{id}", tags=["Movies"], response_model=MovieItem)
...
@movie_router.get("/movies/", tags=["Movies"], response_model=MovieItem)
...
def post_movie(movie: MovieItem): # gets movie following MovieItem model
    try:
...
def put_movie(id: int, movie: MovieItem):
    try:
